...
It requires use of the rapid_map userdefined table type, which consists of the following columns:
Field Name				Field Type				Entry Format				Field Description
point_name				NVARCHAR(MAX)			TEXT								Name of the location
point_description	NVARCHAR(MAX)			TEXT								Description of the location
...
speed							FLOAT							0.0									Speed. Will change colors blue to red when speed is >=3.
																												...
																												makes it easier to distinguish between stationary and moving entities.
*/

/*Create data table*/
DECLARE @rapid_map TABLE (
	point_name NVARCHAR(MAX),
	point_description NVARCHAR(MAX),
...
DECLARE @STYLE1 XML = (
	SELECT 
		'IconStyle01' AS '@id', 
		'ff0000ff' AS 'IconStyle/color', 
		'1' AS 'IconStyle/scale',
		'http://maps.google.com/mapfiles/kml/shapes/shaded_dot.png' AS 'IconStyle/Icon/href',
		'7f0000ff' AS 'PolyStyle/color',
		'0' AS 'LabelStyle/scale' FOR XML PATH ('Style'),
		TYPE
) 
...
DECLARE @FILENAME XML = (
	SELECT CONCAT('Rapid Map : ',FORMAT(GETDATE(),'yyyy-MM-dd HH:mm:ss')) FOR XML PATH('name'),
	TYPE
)
...
DECLARE @TEMP1 TABLE (
	...
	radius_meters FLOAT,
	speed FLOAT,
	BUFFERS_TEXT NVARCHAR(MAX),
	poly_string NVARCHAR(MAX)
)

INSERT INTO @TEMP1 
SELECT 
	s.*, 'poly_string' = REPLACE( REPLACE( REPLACE( REPLACE( s.Buffers_Text, 'Polygon ((', ''), ' ', ','), ',,',',0 ' ), '))', ',0') 
FROM (
...
SELECT 
	...
	(SELECT CONCAT(FORMAT(a.str_datetime,'yyyy-MM-dd'),'T',FORMAT(a.str_datetime,'HH:mm:ss'),'Z') AS 'begin', CONCAT(FORMAT(a.end_datetime,'yyyy-MM-dd'),'T',FORMAT(a.end_datetime,'HH:mm:ss'),'Z') AS 'end' FOR XML PATH('TimeSpan'), TYPE), 
	...
	CAST((SELECT a.poly_string AS 'LinearRing/coordinates' FOR XML PATH('outerBoundaryIs'),Root('Polygon')) AS XML)
FROM @TEMP1 a
...
SELECT (
	SELECT
		...
		(SELECT 
			...
			FOR XML PATH('Placemark'),TYPE) FOR XML PATH('Folder'),
		TYPE
) AS 'loc_output'
FROM @TEMP2 a

/*results*/
DECLARE @QUERYRESULT XML = (
	SELECT 'Locations' AS 'name',
	(SELECT CAST(a.loc_output AS XML) FROM @TEMP3 a FOR XML PATH(''), TYPE) FOR XML PATH ('Folder'),
	TYPE
)
...
DECLARE @FINAL XML = (
	SELECT 
		...
		@STYLE1,
		@STYLE2,
		@QUERYRESULT FOR XML PATH('Document'),root('kml')
)
