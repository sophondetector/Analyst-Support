...
/*Source tables: This is the data to be displayed in i2*/
	DECLARE @CASE TABLE (case_id NVARCHAR(30),case_number NVARCHAR(30),occ_date nvarchar(30),occ_time nvarchar(30),offense NVARCHAR(MAX))
		INSERT INTO @CASE VALUES ('1','24-0001','2024-01-21','12:04','Offense: Assault Offenses'),
	...
	DECLARE @PERS TABLE (pers_id NVARCHAR(30), pers_name NVARCHAR(30),sex NVARCHAR(30))
		INSERT INTO @PERS VALUES('1','Jack','MALE'),
							    ('2','Jill','FEMALE'),
								('3','Jesse','UNKNOWN'),
								('4','Jamie','X')
	DECLARE @CASE_PERS_LINK TABLE (case_id NVARCHAR(30), pers_id NVARCHAR(30)) 
		INSERT INTO @CASE_PERS_LINK VALUES ('1','1'),
										   ('2','1'),
										   ('2','2'),
										   ('3','1'),
										   ('3','2'),
...
DECLARE @OUTPUT TABLE (category NVARCHAR(30),name NVARCHAR(30), content XML,id_type VARCHAR(50),id VARCHAR(34),color_name NVARCHAR(MAX),color_val nvarchar(10))
...
--(i)COLOR_TABLE_GENERATION --https://convertingcolors.com/rgb-color-0_255_255.html is a good converter.
		...
		INSERT INTO @COLR_REFR VALUES('Amethyst','13395609'),
									 ('Beige','14480885'),
									 ('Black','0'),
									 ('Blue','16711680'),
									 ('Bronze','3309517'),
	...
	SELECT  @ET_COL = (
		SELECT		  
		  (SELECT a.id AS '@Id', a.name AS '@Name', a.PreferredRepresentation AS '@PreferredRepresentation', a.IconFile AS '@IconFile'
				,(SELECT col.color_val FROM @COLR_REFR col WHERE col.color_name = a.IconShadingColour) AS '@IconShadingColour' 
			 FROM @entity_type_col_prep a WHERE a.IconShadingColour IS NOT NULL ORDER BY a.id FOR XML PATH('EntityType'),type),
		  (SELECT a.id AS '@Id', a.name AS '@Name', a.PreferredRepresentation AS '@PreferredRepresentation', a.IconFile AS '@IconFile'
...
--LINKTYPECOLLECTION
	...
	SELECT  @LT_COL = (
		SELECT		  
		  (SELECT a.id AS '@Id', a.name AS '@Name', (SELECT col.color_val FROM @COLR_REFR col WHERE col.color_name = a.Colour) AS '@Colour' 
			 FROM @link_type_col_prep a ORDER BY a.id FOR XML PATH('LinkType'),type)
		  	FOR XML PATH('LinkTypeCollection')
		)
--CURRENTSTYLECOLLECTION
	DECLARE @CS_COL XML
	SELECT  @CS_COL = (SELECT(SELECT 'MultiplicitySingle' AS '@Multiplicity' FOR XML PATH('ConnectionStyle'),type)FOR XML PATH('CurrentStyleCollection'))
