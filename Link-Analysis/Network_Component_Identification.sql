...
Allows one to assign referenceable ID's to indidvidual network components containing two or more
network components. Useful when it is desirable to confine analysis to nodes within a specific
network component rather than an entire network. 
POC is:
--Jonathan Miller
--Crime Data Analyst
...
BEGIN
	...
	SET @node = (SELECT node FROM @NODES WHERE CAST(node AS BIGINT) = (SELECT MIN(CAST(node AS BIGINT)) FROM @NODES))
END	
...
DECLARE @MAXCNT INT = ( 
	SELECT MAX(n_count) FROM (
		SELECT nc_id, COUNT(node) as 'n_count' 
			FROM @NC_TABLE
			GROUP BY nc_id
	) s
)
